...
export function SocialLoginButton({ provider, id, requestId, organization, disabled = false }: SocialLoginButtonProps) {
  const config = {
    google: {
      name: "Google",
      icon: (
  ...
  if (disabled) {
    return (
      ...
      >
        {icon}
        <span className="text-gray-400 whitespace-nowrap">Continue with {name} (Not Configured)</span>
      </button>
    );
  }
